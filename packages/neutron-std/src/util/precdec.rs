...
///
/// The greatest possible value that can be represented is 115792089237316195423570985008687907853269984665640.564039457584007913129639935 (which is (2^256 - 1) / 10^27)
...
impl PrecDec {
    ...
    /// Creates a PrecDec(value)
    /// This is equivalent to `PrecDec::from_atomics(value, 27)` but usable in a const context.
    pub const fn new(value: Uint256) -> Self {
        Self(value)
    }

    /// Creates a PrecDec(Uint128(value))
    /// This is equivalent to `PrecDec::from_atomics(value, 27)` but usable in a const context.
    pub const fn raw(value: u128) -> Self {
        Self(Uint256::new(value))
    }

    /// Create a 1.0 PrecDec
    #[inline]
    pub const fn one() -> Self {
        Self(Self::DECIMAL_FRACTIONAL)
    }

    /// Create a 0.0 PrecDec
    #[inline]
    pub const fn zero() -> Self {
        Self(Uint256::zero())
    }

    /// Convert x% into PrecDec
    ///
    /// ## Examples
    ///
    /// ```
    ...
    /// ```
    pub const fn percent(x: u32) -> Self {
        // multiplication does not overflow since `u32::MAX` * 10**25 is well in u128 range
        let atomics = (x as u128) * 10_000_000_000_000_000_000_000_000;
        Self(Uint256::new(atomics))
    }

    /// Convert permille (x/1000) into PrecDec
    ///
    /// ## Examples
    ///
    /// ```
    ...
    pub const fn permille(x: u32) -> Self {
        // multiplication does not overflow since `u32::MAX` * 10**24 is well in u128 range
        let atomics = (x as u128) * 1_000_000_000_000_000_000_000_000;
        Self(Uint256::new(atomics))
    }

    /// Convert basis points (x/10000) into PrecDec
    ///
    /// ## Examples
    ///
    /// ```
    ...
    ///
    /// assert_eq!(TWO_BPS, PrecDec::from_str("0.0002").unwrap());
    /// assert_eq!(HALF, PrecDec::from_str("0.5").unwrap());
    /// ```

    pub const fn bps(x: u32) -> Self {
        // multiplication does not overflow since `u32::MAX` * 10**23 is well in u128 range
        let atomics = (x as u128) * 100_000_000_000_000_000_000_000;
        Self(Uint256::new(atomics))
    }
    ...
    ///
    /// let a = PrecDec::from_atomics(1234u128, 0).unwrap();
    /// assert_eq!(a.to_string(), "1234");
    ///
    /// let a = PrecDec::from_atomics(1u64, 18).unwrap();
    /// assert_eq!(a.to_string(), "0.000000000000000001");
    /// ```
    pub fn from_atomics(
        atomics: impl Into<Uint256>,
        decimal_places: u32,
    ) -> Result<Self, PrecDecRangeExceeded> {
        let atomics = atomics.into();
        const TEN: Uint256 = Uint256::new(10);
        Ok(match decimal_places.cmp(&Self::DECIMAL_PLACES) {
            Ordering::Less => {
                ...
                let factor = TEN.checked_pow(digits).unwrap(); // Safe because digits <= 27
                Self(
                    atomics
                        .checked_mul(factor)
    ...
    /// Returns the ratio (numerator / denominator) as a PrecDec
    ...
    /// assert_eq!(a.atomics(), Uint256::new(1234000000000000000000000000));
    ///
    /// // Smallest possible value
    /// let b = PrecDec::from_str("0.000000000000000000000000001").unwrap();
    /// assert_eq!(b.decimal_places(), 27);
    /// assert_eq!(b.atomics(), Uint256::new(1));
    /// ```
    #[must_use]
    #[inline]
    ...
    /// Returns the approximate square root as a PrecDec.
    ///
...
impl TryFrom<Decimal256> for PrecDec {
    ...
    fn try_from(value: Decimal256) -> Result<Self, Self::Error> {
        Self::from_atomics(value.atomics(), Decimal256::DECIMAL_PLACES)
    }
}
...
impl FromStr for PrecDec {
    ...
    /// Converts the decimal string to a PrecDec
...
impl Mul for PrecDec {
    ...
    fn mul(self, other: Self) -> Self {
        // PrecDecs are fractions. We can multiply two decimals a and b
        // via
...
impl<'de> Deserialize<'de> for PrecDec {
    ...
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_str(PrecDecVisitor)
    }
}

struct PrecDecVisitor;

impl<'de> de::Visitor<'de> for PrecDecVisitor {
    type Value = PrecDec;
...
mod tests {
    ...
    fn precdec_raw() {
        ...
        assert_eq!(PrecDec::raw(value).0, expected);
    }
    ...
    #[test]
    fn precdec_try_from_integer() {
        let int = Uint256::new(999888777666555);
    ...
    fn precdec_from_atomics_works() {
        ...
        // Cuts decimal digits (29 provided but only 27 can be stored)
        assert_eq!(
            PrecDec::from_atomics(4321u128, 29).unwrap(),
            PrecDec::from_str("0.000000000000000000000000043").unwrap()
        );
        assert_eq!(
            PrecDec::from_atomics(6789u128, 29).unwrap(),
            PrecDec::from_str("0.000000000000000000000000067").unwrap()
        );
        assert_eq!(
        ...
        // Overflow is only possible with digits < 26
        let result = PrecDec::from_atomics(Uint256::MAX, 26);
        assert_eq!(result.unwrap_err(), PrecDecRangeExceeded);
    }

    #[test]
    fn precdec_from_ratio_works() {
        // 1.0
        ...
        assert_eq!(
            ...
            PrecDec(Uint256::from(333_333_333_333_333_333_333_333_333u128))
        );

        // 2/3 (result floored)
        assert_eq!(
            PrecDec::from_ratio(2u64, 3u64),
            PrecDec(Uint256::from(666_666_666_666_666_666_666_666_666u128))
        );

        // large inputs
        assert_eq!(PrecDec::from_ratio(0u128, u128::MAX), PrecDec::zero());
        assert_eq!(PrecDec::from_ratio(u128::MAX, u128::MAX), PrecDec::one());
        // 340282366920938463463 is the largest integer <= PrecDec::MAX
        assert_eq!(
    ...
    fn precdec_from_ratio_panics_for_zero_denominator() {
        PrecDec::from_ratio(1u128, 0u128);
    }

    #[test]
    #[should_panic(expected = "Multiplication overflow")]
    fn precdec_from_ratio_panics_for_mul_overflow() {
        PrecDec::from_ratio(Uint256::MAX, 1u128);
    }

    #[test]
    fn precdec_checked_from_ratio_does_not_panic() {
        assert_eq!(
        ...
        assert_eq!(
            PrecDec::checked_from_ratio(Uint256::MAX, 1u128),
            Err(CheckedFromRatioError::Overflow)
        );
    }

    #[test]
    fn precdec_implements_fraction() {
        let fraction = PrecDec::from_str("1234.567").unwrap();
        assert_eq!(
            fraction.numerator(),
            Uint256::from(1_234_567_000_000_000_000_000_000_000_000u128)
        );
        assert_eq!(
            fraction.denominator(),
            Uint256::from(1_000_000_000_000_000_000_000_000_000u128)
        );
    }

    #[test]
    fn precdec_from_str_works() {
        // Integers
        ...
        assert_eq!(
            PrecDec::from_str("7.123456789012345678912345678").unwrap(),
            PrecDec(Uint256::from(7123456789012345678912345678u128))
        );
        assert_eq!(
            PrecDec::from_str("7.999999999999999999999999999").unwrap(),
            PrecDec(Uint256::from(7999999999999999999999999999u128))
        );
    ...
    #[test]
    fn precdec_from_str_errors_for_broken_whole_part() {
        let err = PrecDec::from_str("").unwrap_err();
        assert!(err.to_string().contains("Error parsing whole"));

        let err = PrecDec::from_str(" ").unwrap_err();
        assert!(err.to_string().contains("Error parsing whole"));

        let err = PrecDec::from_str("-1").unwrap_err();
        assert!(err.to_string().contains("Error parsing whole"));
    }

    #[test]
    fn precdec_from_str_errors_for_broken_fractional_part() {
        let err = PrecDec::from_str("1.").unwrap_err();
        assert!(err.to_string().contains("Error parsing fractional"));

        let err = PrecDec::from_str("1. ").unwrap_err();
        assert!(err.to_string().contains("Error parsing fractional"));

        let err = PrecDec::from_str("1.e").unwrap_err();
        assert!(err.to_string().contains("Error parsing fractional"));

        let err = PrecDec::from_str("1.2e3").unwrap_err();
        assert!(err.to_string().contains("Error parsing fractional"));
    }
    ...
    #[test]
    fn precdec_from_str_errors_for_invalid_number_of_dots() {
        let err = PrecDec::from_str("1.2.3").unwrap_err();
        assert!(err.to_string().contains("Unexpected number of dots"));

        let err = PrecDec::from_str("1.2.3.4").unwrap_err();
        assert!(err.to_string().contains("Unexpected number of dots"));
    }
    ...
    fn precdec_decimal_places_works() {
        ...
        assert_eq!(zero.decimal_places(), 27);
        assert_eq!(one.decimal_places(), 27);
        assert_eq!(half.decimal_places(), 27);
        assert_eq!(two.decimal_places(), 27);
        assert_eq!(max.decimal_places(), 27);
    }

    #[test]
    fn precdec_is_zero_works() {
    ...
    fn precdec_inv_works() {
        ...
        assert_eq!(
            ...
            Some(PrecDec::from_str("0.333333333333333333333333333").unwrap())
        );
        assert_eq!(
            PrecDec::from_str("6").unwrap().inv(),
            Some(PrecDec::from_str("0.166666666666666666666666666").unwrap())
        );
    ...
    fn precdec_add_overflow_panics() {
        let _value = PrecDec::MAX + PrecDec::percent(50);
    }

    #[test]
    fn precdec_add_assign_works() {
    ...
    fn precdec_sub_overflow_panics() {
        let _value = PrecDec::zero() - PrecDec::percent(50);
    }

    #[test]
    fn precdec_sub_assign_works() {
    ...
    fn precdec_implements_mul() {
        ...
        assert_eq!(
            max * dec("1.0"),
            dec("115792089237316195423570985008687907853269984665640.564039457584007913129639935")
        );
        assert_eq!(
            max * dec("0.1"),
            dec("11579208923731619542357098500868790785326998466564.056403945758400791312963993")
        );
        assert_eq!(
            max * dec("0.01"),
            dec("1157920892373161954235709850086879078532699846656.405640394575840079131296399")
        );
        assert_eq!(
            max * dec("0.001"),
            dec("115792089237316195423570985008687907853269984665.640564039457584007913129639")
        );
        assert_eq!(
            max * dec("0.000001"),
            dec("115792089237316195423570985008687907853269984.665640564039457584007913129")
        );
        assert_eq!(
            max * dec("0.000000001"),
            dec("115792089237316195423570985008687907853269.984665640564039457584007913")
        );
        assert_eq!(
            max * dec("0.000000000001"),
            dec("115792089237316195423570985008687907853.269984665640564039457584007")
        );
        assert_eq!(
            max * dec("0.000000000000001"),
            dec("115792089237316195423570985008687907.853269984665640564039457584")
        );
        assert_eq!(
            max * dec("0.000000000000000001"),
            dec("115792089237316195423570985008687.907853269984665640564039457")
        );
    ...
    fn precdec_mul_overflow_panics() {
        let _value = PrecDec::MAX * PrecDec::percent(101);
    }

    #[test]
    fn precdec_checked_mul() {
        let test_data = [
    ...
    #[test]
    fn precdec_checked_mul_overflow() {
        assert_eq!(
    ...
    fn precdec_implements_div() {
        ...
        assert_eq!(dec("1") / a, dec("0.00000000000000000812164756"));
        assert_eq!(dec("10") / a, dec("0.000000000000000081216475608"));
        assert_eq!(dec("100") / a, dec("0.000000000000000812164756086"));
    ...
    fn precdec_uint128_divide_by_zero() {
        let left = PrecDec::percent(150); // 1.5
    ...
    #[test]
    #[should_panic(expected = "attempt to divide by zero")]
    fn precdec_uint128_div_assign_by_zero() {
        // a/0
    ...
    /// sqrt(2) is an irrational number, i.e. all 27 decimal places should be used.
    #[test]
    ...
    #[test]
    fn precdec_uint128_sqrt_does_not_overflow() {
        assert_eq!(
    ...
    #[test]
    fn precdec_uint128_sqrt_intermediate_precision_used() {
        assert_eq!(
    ...
    fn precdec_checked_pow() {
        ...
        assert_eq!(
            ...
            PrecDec(10000000000000000000000000u128.into())
        );

        assert_eq!(
            PrecDec::percent(10).checked_pow(27).unwrap(),
            PrecDec(1u128.into())
        );
    }

    #[test]
    fn precdec_checked_pow_overflow() {
        assert_eq!(
    ...
    fn precdec_rem_panics_for_zero() {
        let _ = PrecDec::percent(777) % PrecDec::zero();
    }

    #[test]
    fn precdec_checked_methods() {
        // checked add
        assert_eq!(
            PrecDec::percent(402)
    ...
    #[test]
    #[should_panic]
    fn precdec_pow_overflow_panics() {
        _ = PrecDec::MAX.pow(2u32);
    }

    #[test]
    fn precdec_saturating_works() {
        assert_eq!(
    ...
    fn precdec_ceil_panics() {
        let _ = PrecDec::MAX.ceil();
    }

    #[test]
    fn precdec_checked_ceil() {
        assert_eq!(
    ...
    #[test]
    fn precdec_to_uint_floor_works() {
